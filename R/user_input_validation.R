...
validate_return_object <- function(output, n_empty, item_attributes, performance) {
  ...
  if (performance) {
    # In performance mode, expect three columns: item, attribute, and answer.
    if (ncol(output) != 3 ||
        !("item" %in% colnames(output)) ||
        !("attribute" %in% colnames(output)) ||
        !("answer" %in% colnames(output))) {
      stop(paste("The", string, "must be a data frame with three columns: one named `item`, one named `attribute`, and one named `answer`."))
    }
  } else {
